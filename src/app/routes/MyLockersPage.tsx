...
export const MyLockersPage = () => {
  ...
  const itemCounts = useMemo(() => {
    return myItems.reduce<Record<UserItemType, number>>(
      (acc, item) => {
                  ...
                  {(() => {
                    ...
                    return (
                      <TicketCard
                        ...
                        onClick={
                          useStore ? () => navigate('/store') : () => handleUseTicket('SWAP')
                        }
                        isDisabled={!useStore && !hasLocker}
                        allowZeroAction={useStore}
                        bg={itemBg}
                    ...
                    return (
                      <TicketCard
                        ...
                        onClick={
                          useStore
                            ? () => navigate('/store')
                            : () => handleUseTicket('PENALTY_EXEMPTION')
                        }
                      ...
                      {coinHistories.map((history, index) => (
                        ...
                        >
                          <HStack justify="space-between" align="center">
                            <Stack spacing={1}>
                              <Text fontWeight="semibold">{resolveCoinReason(history.reason)}</Text>
                              <Text fontSize="sm" color={textMuted}>
            ...
            <ModalBody>
              <Stack spacing={4}>
                <HStack spacing={3}>
                  ...
                  <Text fontWeight={returnStep === 'password' ? 'bold' : 'medium'}>
                    비밀번호 입력
                  </Text>
                </HStack>
                {returnStep === 'photo' && (
