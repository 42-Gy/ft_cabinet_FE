...
- **토큰 보안/재발급** – URL `?token=`에서 accessToken을 저장하고, `refresh_token`은 HttpOnly Cookie에 맡깁니다. Axios interceptor가 401 시 `/v4/auth/reissue`를 한 번만 시도한 뒤 원 요청을 재시도합니다.
- **사용자 정보 공유** – `AppProviders`가 `useMeQuery`를 선 실행하여 로그인 직후 새로고침 없이 헤더/사물함/스토어/출석에서 동일 데이터를 공유합니다.
- **스토어 & 아이템 사용** – 대여권/연장권/이사권/감면권을 개별 로딩 상태로 구매하고, /my/lockers에서 “사용” 버튼으로 연장·교체·패널티 감면 API를 호출합니다.
...
> ⚠️ ngrok 백엔드와 동일 Origin(`http://localhost:5173`)으로 접속해야 OAuth 리다이렉트가 성공합니다.
...
- 내 계정 정보, 보유 코인, 현재 사물함 상태, 대여권 구매/반납 버튼을 한 카드에 배치.
...
- 4개의 상품 카드 (대여/연장/이사/감면). 클릭한 카드만 로딩 상태가 나타나 사용자가 혼동하지 않습니다.
...
2. **리다이렉트** – 백엔드가 `http://localhost:5173/?token=ACCESS_TOKEN`으로 리다이렉트하면 `main.tsx`가 쿼리를 파싱하여 `localStorage(subak_access_token)`에 저장하고 주소창을 정리합니다.
...
5. `/my/lockers`: 코인·사물함 정보·아이템 사용 버튼
6. `/store`: 단일 카드만 로딩
